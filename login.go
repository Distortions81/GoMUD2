...
func updateRecentLogins() {
	for _, target := range charList {
		addRecentLogin(target)
	}
}

func addRecentLogin(player *characterData) {
	newLogin := loginData{Name: player.Name, Time: time.Now()}

	for i, item := range recentLogins {
		if item.Name == player.Name {
			recentLogins[i].Time = time.Now()
			return
		}
	}

	recentLogins = append(recentLogins, newLogin)

	if numRecentLogins >= recentLoginMax-1 {
		recentLogins = recentLogins[:recentLoginMax-1]
	} else {
		numRecentLogins++
	}
}
