...
func cmdConInfo(player *characterData, input string) {
	...
	for _, item := range descList {
		...
		player.send("Client: %-28v Charmap: %v", cEllip(strings.Join(item.telnet.termTypeStrings, ","), 28), charmap)
		if item.character != nil {
			player.send("Char: %-30v Account: %v", cEllip(item.character.Name, 30), item.account.Login)
		}

		if item.telnet.Options.HasMTTS {
			buf := ""
			for i, mtts := range MTTSNames {
				if item.telnet.Options.MTTS.hasFlag(i) {
					if buf != "" {
						buf = buf + ", "
					}
					buf = buf + mtts.Name
			...
			player.send("MTTS: %v (%v)", item.telnet.Options.MTTS, buf)
		}
	}
}
