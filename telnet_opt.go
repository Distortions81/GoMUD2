...
var termTypeMap map[string]*termSettings = map[string]*termSettings{
	...
	"AMUDCLIENT": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_TRUECOLOR | 1<<MTTS_UTF8},

	//atlantis Macintosh / OS X, didn't test
	"ATLANTIS": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},

	//beip Worked fine
	"BEIP": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	...
	"GGMUD":    {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	"HARDCOPY": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	"UNKNOWN":  {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	//ggmud

	//kbtin Didn't find any binaries, just source, did not test
	"KBTIN": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},

	//mudlet Works fine
	"MUDLET": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	...
	"MUSHCLIENT": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256},

	//potato Works fine if you never send GA
	"POTATO": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8, SuppressGoAhead: true},

	//POWWOW WINDOWS, uses DOS/OEM/CP437
	"CYGWIN": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256, charMap: charmap.CodePage437},

	//tintin Newline issues on linux?
	"TINTIN": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	...
	"BLOWTORCH": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256},

	//Works, but eats whole lines with accent characters?
	"CMUD": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256},
	...
	//kild works fine
	"KILDCLIENT": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256},
	...
	//Worked fine
	"ZMUD": {MTTS: 1 << MTTS_ANSI},

	//Generic terminal
	"XTERM256COLOR":  {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_UTF8},
	"XTERMTRUECOLOR": {MTTS: 1<<MTTS_ANSI | 1<<MTTS_256 | 1<<MTTS_TRUECOLOR | 1<<MTTS_UTF8},
	"VT100":          {MTTS: 1<<MTTS_ANSI | 1<<MTTS_VT100},
	"ANSI":           {MTTS: 1 << MTTS_ANSI},
	"MONO":           {},
	"DUMB":           {},
